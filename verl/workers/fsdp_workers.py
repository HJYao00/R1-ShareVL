...
class FSDPWorker(Worker):
    ...
    def _init_config(
        ...
        if not hasattr(config, "global_batch_size"):  # ref model
            return
        print(config.global_batch_size)
        if self.config.rollout.n * self.config.rollout.question_num > 1:
            config.global_batch_size = config.global_batch_size * self.config.rollout.n * self.config.rollout.question_num
        ...
        print(config.global_batch_size, self.device_mesh.size(), config.ulysses_sequence_parallel_size, config.global_batch_size_per_device)
