...
class FunctionRewardManager:
    ...
    def __call__(self, data: DataProto) -> Tuple[torch.Tensor, Dict[str, List[float]]]:
        ...
        for i in range(len(data)):
            ...
            # print('res_ids.shape, res_mask.shape', response_ids.shape, response_mask.shape)
            ## res_ids.shape, res_mask.shape torch.Size([2048]) torch.Size([2048])           
            ...
            # print(score)
            ## (Runner pid=537452) {'overall': 0.1, 'format': 1.0, 'accuracy': 0.0}     
