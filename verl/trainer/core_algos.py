...
def compute_grpo_outcome_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, inter_index=None, inter_bool=None, eps: float = 1e-6
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    ...
    inter_id2score = defaultdict(list)
    inter_id2mean, inter_id2std = {}, {}
    ...
    for i in range(bsz):
        ...
        if inter_bool[i] == 1:
            inter_id2score[inter_index[i]].append(scores[i])
    ...
    for idx in inter_id2score:
        inter_id2mean[idx] = torch.mean(torch.tensor(inter_id2score[idx]))
        inter_id2std[idx] = torch.std(torch.tensor(inter_id2score[idx]))

    for i in range(bsz):
        if inter_bool[i] == 1:
            scores[i] = (scores[i] - id2mean[index[i]]) / (id2std[index[i]] + eps) + \
                (scores[i] - inter_id2mean[inter_index[i]]) / (inter_id2std[inter_index[i]] + eps)
        else:
            scores[i] = (scores[i] - id2mean[index[i]]) / (id2std[index[i]] + eps)
