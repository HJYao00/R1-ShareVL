...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0):
    ...
    inter_index = data.non_tensor_batch["inter_uid"]
    inter_bool = data.non_tensor_batch["inter_bool"]
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_algos.compute_grpo_outcome_advantage(token_level_rewards, response_mask, index, inter_index, inter_bool)
...
class RayPPOTrainer:
    ...
    def _create_dataloader(self) -> None:
        self.train_dataset = RLHFDataset(
            ...
            question_num=self.config.worker.rollout.question_num
        )
        ...
        for _ in tqdm(range(self.config.trainer.total_episodes), desc="Episode", position=0):
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                ...
                with _timer("step", timing_raw):
                    ...
                    inter_bs = len(batch.batch)
                    batch = batch.repeat(repeat_times=self.config.worker.rollout.n * self.config.worker.rollout.question_num * self.config.worker.rollout.question_num, interleave=True)

                    # 每个问题一个独立id
                    batch.non_tensor_batch["inter_uid"] = np.array(
                        [str(uuid.uuid4()) for _ in range(inter_bs*self.config.worker.rollout.question_num)], dtype=object
                    )

                    # 
                    def generate_diagonal_list(n):
                        result = []
                        for i in range(n):
                            for j in range(n):
                    ...
                    batch.non_tensor_batch['inter_bool'] = np.array(generate_diagonal_list(self.config.worker.rollout.question_num), dtype=object)
                    batch.non_tensor_batch['inter_bool'] = np.tile(batch.non_tensor_batch['inter_bool'], inter_bs)

                    batch.non_tensor_batch["inter_uid"] = np.repeat(batch.non_tensor_batch["inter_uid"], self.config.worker.rollout.n * self.config.worker.rollout.question_num)
                    batch.non_tensor_batch['inter_bool'] = np.repeat(batch.non_tensor_batch['inter_bool'], self.config.worker.rollout.n)

                    assert len(batch.non_tensor_batch["inter_uid"]) == len(batch.non_tensor_batch["inter_bool"]) == len(batch.non_tensor_batch["uid"])
